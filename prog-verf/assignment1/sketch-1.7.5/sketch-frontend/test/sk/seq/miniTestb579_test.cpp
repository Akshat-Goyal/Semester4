...
void boo_ANONYMOUSTest(Parameters& _p_) {
  for(int _test_=0;_test_< _p_.niters ;_test_++) {
    int  x;
    x=abs(rand()) % 32;
    if(_p_.verbosity > 2){
    ...
    try{
      ANONYMOUS::foo(x,_out_outsk);
      ANONYMOUS::boo(x,_out_outsp);
    }catch(AssumptionFailedException& afe){  }
    if(_out_outsk!=_out_outsp) {
      printf("Automated testing failed in boo_ANONYMOUSTest\n");
      cout<<"x = "<<x<<endl;
      cout<<"_out_outsk = "<<_out_outsk<<endl;
      cout<<"_out_outsp = "<<_out_outsp<<endl;
      exit(1);
...
int main(int argc, char** argv) {
  ...
  moo_ANONYMOUSTest(p);
  printf("Automated testing passed for miniTestb579\n");
  return 0;
}
