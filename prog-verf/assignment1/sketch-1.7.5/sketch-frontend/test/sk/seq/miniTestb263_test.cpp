...
void testSK9_ANONYMOUSTest(Parameters& _p_) {
  for(int _test_=0;_test_< _p_.niters ;_test_++) {
    ...
    try{
      ANONYMOUS::test9(N,in,_out_outsk);
      ANONYMOUS::testSK9(N,in,_out_outsp);
    }catch(AssumptionFailedException& afe){  }
    for(int _i_=0;_i_<N;_i_++) {
      if(_out_outsk[_i_]!=_out_outsp[_i_]) {
        printf("Automated testing failed in testSK9_ANONYMOUSTest\n");
        cout<<"N = "<<N<<endl;
...
int main(int argc, char** argv) {
  ...
  srand(time(0));
  testSK9_ANONYMOUSTest(p);
  printf("Automated testing passed for miniTestb263\n");
  return 0;
}
