...
scoring_matrix = {'A': {'A': 4, 'C': -1, 'G': 1, 'T': -1}, 'C': {'A': -1, 'C': 4, 'G': -1,
                                                                 'T': 1}, 'G': {'A': 1, 'C': -1, 'G': 4, 'T': -1}, 'T': {'A': -1, 'C': 1, 'G': -1, 'T': 4}}
pgap = -3
...
def globalAlignment(s, t):
    ...
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            dp[i][j] = max(dp[i-1][j-1] + scoring_matrix[s[i-1]]
                           [t[j-1]], dp[i][j-1] + pgap, dp[i-1][j] + pgap)
    gs = ""
    gt = ""
    i, j = n, m
    while i > 0 or j > 0:
...
def localAlignment(s, t):
    ...
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            dp[i][j] = max(dp[i-1][j-1] + scoring_matrix[s[i-1]]
                           [t[j-1]], dp[i][j-1] + pgap, dp[i-1][j] + pgap, 0)
    gs = ""
    gt = ""
    i, j = 0, 0
    score = 0
